...
Module Host has incubating library slices: `punk-module-host` can preflight a pure module invocation envelope, wrap advisory module assessment output, model future module receipt field coverage, model future side-effect request preconditions, preflight future policy evidence readiness, preflight future side-effect receipt writer readiness, model future receipt-writer active behavior outcomes, model future receipt-writer file IO plans, model target/storage policy readiness, model redacted host path observations, compose concrete path/storage policy readiness, and model operation-evidence persistence readiness without performing those side effects. It does not load plugins, invoke modules, expose CLI behavior, resolve or canonicalize host paths, read or write files, create or write receipts, persist operation evidence, mutate event logs, invoke policy engines, invoke gate, call APIs, read credentials, invoke adapters, publish, comment, create pull requests, or write gate/proof authority.
