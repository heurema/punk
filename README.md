...
Module Host has one incubating library slice: `punk-module-host` can preflight a pure module invocation envelope and wrap advisory module assessment output. It does not load plugins, invoke modules, expose CLI behavior, read or write files, create receipts, mutate event logs, call APIs, read credentials, invoke adapters, publish, or write gate/proof authority.
