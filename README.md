...
Module Host has incubating library slices: `punk-module-host` can preflight a pure module invocation envelope, wrap advisory module assessment output, model future module receipt field coverage, model future side-effect request preconditions, and preflight future policy evidence readiness without performing those side effects. It does not load plugins, invoke modules, expose CLI behavior, read or write files, create receipts, mutate event logs, invoke policy engines, invoke gate, call APIs, read credentials, invoke adapters, publish, comment, create pull requests, or write gate/proof authority.
