...
use punk_events::{
    append_local_flow_event, append_local_receipt_evidence_handoff_event, schema_fixture,
    EventCorrelation, EventKind, EventSource, EventStatus, MemoryEventLog,
};
...
use punk_mod_pubpunk::{
    ...
    PubPunkChannelConnectorProfileResolutionPacket, PubPunkConnectorStrategy,
    PubPunkConnectorStrategySelectionReason, PubPunkInventoryInput, PubPunkInventoryInputPacket,
    PubPunkInventoryItemInput, PubPunkInventoryItemKind, PubPunkInventoryItemStatus,
    PubPunkInventoryReaderInput, PubPunkPublishOperationEvidenceHandoffPacket,
    PubPunkPublishReceiptEvidenceEventHandoffPacket, PubPunkPublishReceiptPreflightPacket,
    PubPunkPublishReceiptWriteHandoffPacket, PubPunkPublishRequestPacket, PubPunkWorkspacePolicy,
...
use punk_module_host::{
    ...
    module_side_effect_receipt_writer_operation_evidence_write_slice_boundary,
    ...
    propose_module_assessment_receipt, propose_module_side_effect_request, wrap_module_assessment,
    write_module_side_effect_receipt_first_active_slice,
    write_module_side_effect_receipt_writer_operation_evidence_first_active_slice,
    ModuleCapabilityGrant, ModuleHostStatus, ModuleInvocationEnvelope, ModuleOutputAuthority,
    ModuleOutputBoundaryFlags, ModuleOutputStatus, ModuleOutputSummary,
    ModulePolicyGatePreflightDraft, ModulePolicyGatePreflightRequirement,
    ...
    ModuleSideEffectReceiptWriterOperationEvidenceWriteSliceBlocker,
    ...
    MODULE_HOST_SIDE_EFFECT_RECEIPT_WRITER_OPERATION_EVIDENCE_WRITE_SLICE_SCHEMA_VERSION,
};
use punk_project::{
...
pub fn run_smoke_suite() -> SmokeEvalReport {
    let cases = vec![
        ...
        eval_local_receipt_evidence_handoff_appends_event_refs(),
        ...
        eval_pubpunk_inventory_reader_builds_packet_from_explicit_refs_without_side_effects(),
        eval_pubpunk_inventory_input_packet_is_explicit_and_side_effect_free(),
        ...
        eval_module_host_side_effect_receipt_writer_file_io_plan_model_is_side_effect_free(),
        eval_module_host_side_effect_receipt_writer_target_storage_policy_model_is_side_effect_free(
        ),
        eval_module_host_side_effect_receipt_writer_host_path_observation_model_is_side_effect_free(
        ),
        ...
        eval_module_host_side_effect_receipt_writer_first_active_write_slice_writes_receipt_bytes(),
        eval_module_host_side_effect_receipt_writer_operation_evidence_write_slice_writes_evidence_bytes(),
    ...
    SmokeEvalReport {
        ...
        boundary_notes: vec![
            ...
            "local receipt/evidence event handoff smoke coverage appends only safe receipt and operation-evidence refs to .punk/events/flow.jsonl without writing .punk/runs artifacts or gate/proof authority",
            ...
            "PubPunk inventory reader smoke case builds an input packet from explicit observed refs without filesystem walks, draft body reads, workspace initialization, publishing, metrics collection, receipt writing, adapter invocation, or gate/proof authority",
            "PubPunk inventory input packet smoke case requires explicit workspace, instruction, source, capability, receipt-field, and token-cost refs before assessment without reading files, publishing, creating receipts, invoking adapters, or writing gate/proof authority",
            ...
            "module-host side-effect receipt writer file IO plan smoke case models storage refs, receipt target refs, target path refs, write policy, idempotency, temp/atomic policy, rollback/error visibility, and operation-evidence persistence visibility without writing receipts, mutating event logs, persisting operation evidence, reading or writing files, invoking adapters, invoking policy engines, invoking gate, publishing, commenting, creating pull requests, calling APIs, reading credentials, writing proofpacks, or claiming acceptance",
            ...
            "module-host side-effect receipt writer first active write slice smoke case writes exact receipt bytes only to an explicit temporary .punk/runs target without persisting operation evidence, mutating event logs, invoking adapters, invoking policy engines, invoking gate, publishing, commenting, creating pull requests, calling APIs, reading credentials, writing proofpacks, or claiming acceptance",
            "module-host side-effect receipt writer operation-evidence write slice smoke case writes exact operation-evidence bytes only to an explicit temporary .punk/runs target after a successful receipt write without mutating event logs, invoking adapters, invoking policy engines, invoking gate, publishing, commenting, creating pull requests, calling APIs, reading credentials, writing proofpacks, or claiming acceptance",
...
fn eval_local_receipt_evidence_handoff_appends_event_refs() -> SmokeEvalCaseResult {
    let temp_path = unique_smoke_temp_path();
    if let Err(error) = fs::create_dir_all(temp_path.join(".punk")) {
        return SmokeEvalCaseResult::fail(
            "eval_local_receipt_evidence_handoff_appends_event_refs",
            "local receipt/evidence handoff appends bounded event refs",
            format!("temporary project marker setup failed with {error:?}"),
        );
    }
    if let Err(error) = fs::write(
    ...
    let receipt_ref = ".punk/runs/pubpunk-publish-community-lab/receipt.json";
    let operation_evidence_ref =
        ".punk/runs/pubpunk-publish-community-lab/operation-evidence.jsonl";
    let append_result = append_local_receipt_evidence_handoff_event(
        &temp_path,
        EventSource::new("punk-events", "receipt_evidence_handoff"),
        EventCorrelation::new("flow_local_receipt_evidence")
    ...
    let result = match (append_result, rendered) {
        (Ok(record), Ok(text))
            if record.sequence == 1
                && record.event_id == "evt_0000000000000001"
                && record.kind == EventKind::ReceiptEvidenceHandoff
                && record.result.status == EventStatus::Succeeded
                && record.artifacts.refs == vec![receipt_ref.to_owned(), operation_evidence_ref.to_owned()]
                && text.lines().count() == 1
                && text.contains("\"kind\":\"receipt_evidence_handoff\"")
                && text.contains(receipt_ref)
                && text.contains(operation_evidence_ref)
                && !text.contains("decision_id")
                && !forbidden_runtime_created =>
        {
            SmokeEvalCaseResult::pass(
                "eval_local_receipt_evidence_handoff_appends_event_refs",
                "local receipt/evidence handoff appends bounded event refs",
                "local receipt/evidence handoff appended safe refs to .punk/events/flow.jsonl without writing .punk/runs artifacts, decisions, proofs, adapters, publishing, comments, pull requests, or acceptance",
            )
        }
        (Ok(record), Ok(text)) => SmokeEvalCaseResult::fail(
            "eval_local_receipt_evidence_handoff_appends_event_refs",
            "local receipt/evidence handoff appends bounded event refs",
            format!(
                "local receipt/evidence handoff drifted; event_id={} sequence={} kind={} lines={} forbidden_runtime_created={}",
                record.event_id,
                record.sequence,
                record.kind.as_str(),
                text.lines().count(),
        ...
        (Err(error), _) => SmokeEvalCaseResult::fail(
            "eval_local_receipt_evidence_handoff_appends_event_refs",
            "local receipt/evidence handoff appends bounded event refs",
            format!("local receipt/evidence event append failed with {error:?}"),
        ),
        (_, Err(error)) => SmokeEvalCaseResult::fail(
            "eval_local_receipt_evidence_handoff_appends_event_refs",
            "local receipt/evidence handoff appends bounded event refs",
            format!("local receipt/evidence event log read failed with {error:?}"),
        ),
    };
...
fn eval_pubpunk_inventory_reader_builds_packet_from_explicit_refs_without_side_effects(
) -> SmokeEvalCaseResult {
    let reader_input = PubPunkInventoryReaderInput::new(
        "v0.1",
        "contracts/pubpunk-inventory-cycle-0",
        "runs/pubpunk-inventory-reader",
        "project/punk",
    ...
    .with_allowed_source_refs(vec![
        ...
        "publishing/metrics/community-lab.md",
        "publishing/misc/unknown.md",
    ])
    ...
    .with_token_cost_ref("work/reports/pubpunk-inventory-reader-token-cost.md");

    let reader_assessment = assess_pubpunk_inventory_reader_input(&reader_input);
    let packet = match build_pubpunk_inventory_input_packet_from_reader_input(&reader_input) {
        Ok(packet) => packet,
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_inventory_reader_builds_packet_from_explicit_refs_without_side_effects",
                "PubPunk inventory reader builds packet from explicit refs without side effects",
                "ready PubPunk inventory reader input failed to build an input packet",
            );
        }
    };
    let packet_assessment = assess_pubpunk_inventory_input_packet(&packet);
    let inventory_input = match packet.try_into_inventory_input() {
        Ok(input) => input,
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_inventory_reader_builds_packet_from_explicit_refs_without_side_effects",
                "PubPunk inventory reader builds packet from explicit refs without side effects",
                "reader-built PubPunk input packet failed to convert into inventory assessment input",
            );
        }
    };
    let inventory_assessment = assess_pubpunk_inventory(&inventory_input);

    let empty_reader_input = PubPunkInventoryReaderInput::new(
        "v0.1",
        "contracts/pubpunk-inventory-cycle-0",
        "runs/pubpunk-inventory-reader-empty",
        "project/new",
        "punk-publishing://project/new",
    )
    .with_instruction_refs(PUBPUNK_REQUIRED_INSTRUCTION_REFS.to_vec())
    ...
    let empty_reader_assessment = assess_pubpunk_inventory_reader_input(&empty_reader_input);
    let empty_packet = build_pubpunk_inventory_input_packet_from_reader_input(&empty_reader_input);

    let ok = reader_assessment.status == PubPunkAssessmentStatus::Ready
        && reader_assessment.authority == PubPunkAssessmentAuthority::Advisory
        && !reader_assessment.has_blockers()
        && reader_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && reader_assessment.refs.module_id == "pubpunk"
        && reader_assessment.refs.workspace_policy == PubPunkWorkspacePolicy::SplitExplicitRefs
        && reader_assessment.refs.publishing_workspace_ref == "punk-publishing://project/punk"
        && reader_assessment.refs.token_cost_ref.as_deref()
            == Some("work/reports/pubpunk-inventory-reader-token-cost.md")
        && reader_assessment.observed_item_count == 4
        && reader_assessment.candidate_count == 1
        && reader_assessment.receipt_count == 1
        && reader_assessment.metrics_snapshot_count == 1
        && reader_assessment.unclear_item_count == 1
        && packet.granted_capabilities == vec![PubPunkCapabilityGrant::AssessProvidedInventory]
        && packet.items.len() == 4
        && packet_assessment.status == PubPunkAssessmentStatus::Ready
        && !packet_assessment.has_blockers()
        && packet_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && inventory_assessment.status == PubPunkAssessmentStatus::Ready
        && !inventory_assessment.has_blockers()
        && inventory_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && empty_reader_assessment.status == PubPunkAssessmentStatus::Ready
        && empty_reader_assessment.observed_item_count == 0
        && empty_reader_assessment.candidate_count == 0
        && empty_reader_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && empty_packet.is_ok();
    ...
    if ok {
        SmokeEvalCaseResult::pass(
            "eval_pubpunk_inventory_reader_builds_packet_from_explicit_refs_without_side_effects",
            "PubPunk inventory reader builds packet from explicit refs without side effects",
            "PubPunk inventory reader modeled explicit observed refs, allowed-source boundaries, empty new-project inventory, and packet construction without filesystem walks, draft body reads, workspace initialization, publishing, metrics collection, receipt writing, adapter invocation, gate/proof authority, or acceptance claims",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_pubpunk_inventory_reader_builds_packet_from_explicit_refs_without_side_effects",
            "PubPunk inventory reader builds packet from explicit refs without side effects",
            "PubPunk inventory reader drifted from explicit observed-ref packet-building boundary",
        )
    }
}

fn eval_pubpunk_inventory_input_packet_is_explicit_and_side_effect_free() -> SmokeEvalCaseResult {
    let packet = PubPunkInventoryInputPacket::new(
        "v0.1",
        "contracts/pubpunk-inventory-cycle-0",
    ...
    .with_instruction_refs(PUBPUNK_REQUIRED_INSTRUCTION_REFS.to_vec())
    .with_allowed_source_refs(vec!["publishing/posts/community-lab.md"])
    ...
    .with_channel("github-discussions")])
    .with_token_cost_ref("work/reports/pubpunk-inventory-token-cost.md");

    let packet_assessment = assess_pubpunk_inventory_input_packet(&packet);
    let inventory_input = packet.try_into_inventory_input();
    let ok = packet_assessment.status == PubPunkAssessmentStatus::Ready
        && packet_assessment.authority == PubPunkAssessmentAuthority::Advisory
        && !packet_assessment.has_blockers()
        && packet_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && packet_assessment.refs.module_id == "pubpunk"
        && packet_assessment.refs.workspace_policy == PubPunkWorkspacePolicy::SplitExplicitRefs
        && packet_assessment.refs.publishing_workspace_ref == "punk-publishing://project/punk"
        && packet_assessment.refs.token_cost_ref.as_deref()
            == Some("work/reports/pubpunk-inventory-token-cost.md")
        && inventory_input.is_ok();

    if ok {
        SmokeEvalCaseResult::pass(
            "eval_pubpunk_inventory_input_packet_is_explicit_and_side_effect_free",
            "PubPunk inventory input packet requires explicit refs before assessment",
            "PubPunk inventory input packet validated explicit workspace, instruction, source, capability, receipt-field, and token-cost refs without filesystem, CLI, external API, credential, adapter, receipt writer, gate, proof, or acceptance behavior",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_pubpunk_inventory_input_packet_is_explicit_and_side_effect_free",
            "PubPunk inventory input packet requires explicit refs before assessment",
            "PubPunk inventory input packet drifted from explicit advisory no-IO module boundary",
        )
    }
}
...
fn eval_pubpunk_channel_connector_profile_resolution_selects_strategy_without_side_effects(
) -> SmokeEvalCaseResult {
    let api_packet = PubPunkChannelConnectorProfileResolutionPacket::new(
        "v0.1",
    ...
    let api_assessment = assess_pubpunk_channel_connector_profile_resolution_packet(&api_packet);
    let api_refs = match api_packet.try_into_connector_profile_resolution_refs() {
        Ok(refs) => refs,
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_channel_connector_profile_resolution_selects_strategy_without_side_effects",
                "PubPunk connector profile resolution selects API, browser, or manual without side effects",
                "ready PubPunk connector profile resolution packet failed to build selected connector refs",
            );
        }
    };

    let browser_packet = api_packet.clone().with_api_available(false);
    let manual_packet = api_packet
        .clone()
        .with_api_available(false)
        .with_browser_allowed(false);
    let blocked_packet = api_packet
        .with_api_available(false)
        .with_browser_allowed(false)
    ...
    let blocked_assessment =
        assess_pubpunk_channel_connector_profile_resolution_packet(&blocked_packet);

    let ok = api_assessment.status == PubPunkAssessmentStatus::Ready
        && api_assessment.authority == PubPunkAssessmentAuthority::Advisory
        && !api_assessment.has_blockers()
        && api_assessment.boundary_flags.all_side_effect_flags_false()
        && api_assessment.selected_strategy == Some(PubPunkConnectorStrategy::Api)
        && api_assessment.selection_reason
            == PubPunkConnectorStrategySelectionReason::ApiPreferredAndAvailable
        && api_assessment.refs.connector_profile_ref
            == "publishing/connectors/github-discussions-community-lab.md"
        && api_assessment.refs.credential_signal_ref
            == "publishing/connectors/github-discussions-credential-signal.md"
        && api_assessment.refs.token_cost_ref.as_deref()
            == Some("work/reports/pubpunk-connector-profile-resolution-token-cost.md")
        && api_refs.selected_strategy == PubPunkConnectorStrategy::Api
        && api_refs.channel_ref == "publishing/channels/github-discussions-community-lab.md"
        && api_refs.connector_profile_ref
            == "publishing/connectors/github-discussions-community-lab.md"
        && browser_assessment.status == PubPunkAssessmentStatus::Ready
        && browser_assessment.selected_strategy == Some(PubPunkConnectorStrategy::Browser)
        && browser_assessment.selection_reason
            == PubPunkConnectorStrategySelectionReason::BrowserFallbackAllowed
        && browser_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && manual_assessment.status == PubPunkAssessmentStatus::Ready
        && manual_assessment.selected_strategy == Some(PubPunkConnectorStrategy::Manual)
        && manual_assessment.selection_reason
            == PubPunkConnectorStrategySelectionReason::ManualFallbackAllowed
        && manual_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && blocked_assessment.status == PubPunkAssessmentStatus::Blocked
        && blocked_assessment.selected_strategy.is_none()
        && blocked_assessment.selection_reason
    ...
    if ok {
        SmokeEvalCaseResult::pass(
            "eval_pubpunk_channel_connector_profile_resolution_selects_strategy_without_side_effects",
            "PubPunk connector profile resolution selects API, browser, or manual without side effects",
            "PubPunk connector profile resolution selected API first, browser automation only when API was unavailable and browser policy allowed it, and manual handoff when both automated paths were unavailable, using explicit refs without calling APIs, opening browsers, reading credentials, invoking adapters, publishing, collecting metrics, writing receipts, or activating runtime",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_pubpunk_channel_connector_profile_resolution_selects_strategy_without_side_effects",
            "PubPunk connector profile resolution selects API, browser, or manual without side effects",
            "PubPunk connector profile resolution drifted from explicit API-first, browser-policy, manual-fallback no-execution boundary",
        )
    }
}
...
fn eval_pubpunk_host_handoff_chains_packet_to_receipt_proposal_without_side_effects(
) -> SmokeEvalCaseResult {
    let packet = PubPunkInventoryInputPacket::new(
        "v0.1",
        "contracts/pubpunk-inventory-cycle-0",
    ...
    let inventory_input = match packet.try_into_inventory_input() {
        ...
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_host_handoff_chains_packet_to_receipt_proposal_without_side_effects",
                "PubPunk host handoff chains packet readiness to receipt proposal without side effects",
                "ready PubPunk input packet failed to convert into inventory assessment input",
            );
        }
    };
    ...
    let ok = packet_assessment.status == PubPunkAssessmentStatus::Ready
        ...
        && packet_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && packet_assessment.refs.module_id == invocation.module_id
        && packet_assessment.refs.contract_ref == invocation.contract_ref
        && packet_assessment.refs.run_ref == invocation.run_ref
        && packet_assessment.refs.project_ref == invocation.project_ref
        && packet_assessment.refs.workspace_policy == PubPunkWorkspacePolicy::SplitExplicitRefs
        && packet_assessment.refs.publishing_workspace_ref == "punk-publishing://project/punk"
        && packet_assessment.refs.token_cost_ref.as_deref()
            == Some("work/reports/pubpunk-inventory-token-cost.md")
        && inventory_input.requested_operation.as_str() == invocation.requested_operation
        && pubpunk_assessment.status == PubPunkAssessmentStatus::Ready
        && pubpunk_assessment.authority == PubPunkAssessmentAuthority::Advisory
        && !pubpunk_assessment.has_blockers()
        && pubpunk_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && pubpunk_assessment.refs.module_id == invocation.module_id
        && preflight.status == ModuleHostStatus::Ready
        && !preflight.has_blockers()
        && preflight.boundary_flags.all_side_effect_flags_false()
        && envelope.status == ModuleHostStatus::Ready
        && !envelope.has_blockers()
        && envelope.module_id == invocation.module_id
        && envelope.module_output_status == ModuleOutputStatus::Ready
        && envelope.module_output_ref == "work/module-assessments/pubpunk-inventory.md"
        && envelope.boundary_flags.all_side_effect_flags_false()
        && proposal.status == ModuleHostStatus::Ready
        && !proposal.has_blockers()
        && proposal.module_id == invocation.module_id
        && proposal.module_output_ref == envelope.module_output_ref
        && proposal
            .covered_fields
            .contains(&ModuleReceiptProposalField::HostValidation)
        && proposal
    ...
    if ok {
        SmokeEvalCaseResult::pass(
            "eval_pubpunk_host_handoff_chains_packet_to_receipt_proposal_without_side_effects",
            "PubPunk host handoff chains packet readiness to receipt proposal without side effects",
            "PubPunk host handoff chained input packet readiness, inventory assessment, module-host preflight, advisory envelope, and receipt proposal without invoking modules, reading files, writing receipts, publishing, collecting metrics, invoking adapters, writing gate/proof authority, or claiming acceptance",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_pubpunk_host_handoff_chains_packet_to_receipt_proposal_without_side_effects",
            "PubPunk host handoff chains packet readiness to receipt proposal without side effects",
            "PubPunk host handoff drifted from the packet-to-assessment-to-host-proposal advisory boundary",
        )
    }
}

fn eval_pubpunk_publish_request_packet_chains_to_host_side_effect_request() -> SmokeEvalCaseResult {
    let reader_input = PubPunkInventoryReaderInput::new(
        "v0.1",
        "contracts/pubpunk-publish-cycle-0",
        "runs/pubpunk-publish-request",
        "project/punk",
        "punk-publishing://project/punk",
    )
    .with_instruction_refs(PUBPUNK_REQUIRED_INSTRUCTION_REFS.to_vec())
    ...
    let packet = match build_pubpunk_inventory_input_packet_from_reader_input(&reader_input) {
        ...
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                ...
                "ready reader input failed to build inventory input packet",
            );
        }
    };
    let inventory_input = match packet.try_into_inventory_input() {
        ...
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_publish_request_packet_chains_to_host_side_effect_request",
                "PubPunk publish request packet chains to host side-effect request",
                "ready inventory input packet failed to convert into inventory input",
            );
        }
    };
    let pubpunk_assessment = assess_pubpunk_inventory(&inventory_input);
    ...
    let side_effect_refs = match publish_packet.try_into_side_effect_request_refs() {
        Ok(refs) => refs,
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_publish_request_packet_chains_to_host_side_effect_request",
                "PubPunk publish request packet chains to host side-effect request",
                "ready PubPunk publish request packet failed to build side-effect request refs",
            );
        }
    };
    ...
    let output = ModuleOutputSummary::new(
        ...
        ModuleOutputStatus::Ready,
        ModuleOutputAuthority::Advisory,
        publish_packet_assessment.findings.len(),
        ModuleOutputBoundaryFlags::side_effect_free(),
    );
    let envelope = wrap_module_assessment(&invocation, &output);
    let receipt_proposal = propose_module_assessment_receipt(&invocation, &envelope);
    let request = ModuleSideEffectRequestDraft::new(
        side_effect_refs.request_ref.clone(),
        ModuleSideEffectKind::Publish,
    )
    .with_target_ref(side_effect_refs.target_ref.clone())
    .with_intent_ref(side_effect_refs.intent_ref.clone())
    .with_policy_ref(side_effect_refs.policy_ref.clone())
    .with_receipt_proposal_ref(side_effect_refs.receipt_proposal_ref.clone())
    .with_adapter_ref(side_effect_refs.adapter_ref.clone())
    .with_payload_ref(side_effect_refs.payload_ref.clone());
    ...
    .with_side_effect_receipt_proposal_ref(side_effect_refs.receipt_proposal_ref.clone())
    ...
    let ok = reader_assessment.status == PubPunkAssessmentStatus::Ready
        ...
        && publish_packet_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && publish_packet_assessment.refs.module_id == "pubpunk"
        ...
        && side_effect_proposal.request_id
            == "work/module-side-effects/pubpunk-publish-community-lab.md"
        && side_effect_proposal.target_ref
            == "publishing/channels/github-discussions-community-lab.md"
        && side_effect_proposal.payload_ref == "publishing/posts/community-lab.md"
    ...
    if ok {
        ...
        SmokeEvalCaseResult::fail(
            "eval_pubpunk_publish_request_packet_chains_to_host_side_effect_request",
            "PubPunk publish request packet chains to host side-effect request",
            "PubPunk publish request packet drifted from explicit no-execution side-effect request boundary",
        )
    }
}

fn eval_pubpunk_publish_receipt_preflight_chains_to_host_receipt_writer_preflight(
) -> SmokeEvalCaseResult {
    let reader_input = PubPunkInventoryReaderInput::new(
        "v0.1",
        "contracts/pubpunk-publish-cycle-0",
    ...
    let packet = match build_pubpunk_inventory_input_packet_from_reader_input(&reader_input) {
        ...
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_publish_receipt_preflight_chains_to_host_receipt_writer_preflight",
                "PubPunk publish receipt preflight chains to host receipt writer preflight",
                "ready reader input failed to build inventory input packet",
            );
        }
    };
    ...
    let invocation_preflight = preflight_module_invocation(&invocation);
    let output = ModuleOutputSummary::new(
        "work/module-assessments/pubpunk-publish-request.md",
        ModuleOutputStatus::Ready,
        ModuleOutputAuthority::Advisory,
    ...
    let receipt_preflight_packet = PubPunkPublishReceiptPreflightPacket::new(
        "v0.1",
        "contracts/pubpunk-publish-cycle-0",
        "runs/pubpunk-publish-request",
        "project/punk",
    ...
    .with_receipt_writer_preflight_ref(
        "work/module-receipt-preflights/pubpunk-publish-community-lab.md",
    )
    .with_policy_gate_preflight_ref(policy_gate_preflight.preflight_id.clone())
    .with_receipt_target_ref(side_effect_refs.receipt_proposal_ref.clone())
    .with_receipt_storage_ref(".punk/runs/pubpunk-publish-request/receipts")
    .with_operation_evidence_ref(
        ".punk/runs/pubpunk-publish-request/operation-evidence/pubpunk-publish-community-lab.md",
    )
    .with_idempotency_ref("work/module-idempotency/pubpunk-publish-community-lab.md")
    .with_rollback_ref("work/module-rollbacks/pubpunk-publish-community-lab.md")
    .with_error_ref("work/module-errors/pubpunk-publish-community-lab.md")
    .with_adapter_invocation_receipt_ref(
        policy_gate_preflight.adapter_invocation_receipt_ref.clone(),
    )
    .with_payload_ref(side_effect_refs.payload_ref.clone())
    .with_channel_ref(side_effect_refs.target_ref.clone())
    .with_connector_profile_resolution_ref(
        side_effect_refs.connector_profile_resolution_ref.clone(),
    )
    .with_connector_profile_ref(side_effect_refs.connector_profile_ref.clone())
    .with_selected_connector_strategy_ref(side_effect_refs.selected_connector_strategy_ref.clone())
    .with_instruction_refs(PUBPUNK_REQUIRED_INSTRUCTION_REFS.to_vec())
    .with_allowed_source_refs(vec![
        "publishing/posts/community-lab.md",
        "publishing/channels/github-discussions-community-lab.md",
    ...
    .with_token_cost_ref("work/reports/pubpunk-publish-receipt-preflight-token-cost.md");
    let receipt_preflight_packet_assessment =
        assess_pubpunk_publish_receipt_preflight_packet(&receipt_preflight_packet);
    let receipt_writer_refs = match receipt_preflight_packet
        .try_into_receipt_writer_preflight_refs()
    {
        ...
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_publish_receipt_preflight_chains_to_host_receipt_writer_preflight",
                "PubPunk publish receipt preflight chains to host receipt writer preflight",
                "ready PubPunk publish receipt preflight packet failed to build host writer preflight refs",
            );
        }
    };

    let receipt_writer_draft =
        ModuleSideEffectReceiptWriterPreflightDraft::new(receipt_writer_refs.preflight_ref.clone())
            .with_policy_gate_preflight_ref(receipt_writer_refs.policy_gate_preflight_ref.clone())
            .with_receipt_target_ref(receipt_writer_refs.receipt_target_ref.clone())
            .with_storage_ref(receipt_writer_refs.storage_ref.clone())
            .with_operation_evidence_ref(receipt_writer_refs.operation_evidence_ref.clone())
            .with_idempotency_ref(receipt_writer_refs.idempotency_ref.clone())
            .with_rollback_ref(receipt_writer_refs.rollback_ref.clone())
            .with_error_ref(receipt_writer_refs.error_ref.clone())
            .with_adapter_invocation_receipt_ref(
                receipt_writer_refs.adapter_invocation_receipt_ref.clone(),
            )
            .with_payload_ref(receipt_writer_refs.payload_ref.clone());
    ...
    let ok = reader_assessment.status == PubPunkAssessmentStatus::Ready
        && pubpunk_assessment.status == PubPunkAssessmentStatus::Ready
        && publish_packet_assessment.status == PubPunkAssessmentStatus::Ready
        && receipt_preflight_packet_assessment.status == PubPunkAssessmentStatus::Ready
        && receipt_preflight_packet_assessment.authority == PubPunkAssessmentAuthority::Advisory
        && !receipt_preflight_packet_assessment.has_blockers()
        && receipt_preflight_packet_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && receipt_preflight_packet_assessment
            .refs
        ...
        && invocation_preflight.status == ModuleHostStatus::Ready
        && envelope.status == ModuleHostStatus::Ready
        && receipt_proposal.status == ModuleHostStatus::Ready
        && side_effect_proposal.status == ModuleHostStatus::Ready
        && policy_gate_preflight.status == ModuleHostStatus::Ready
        && receipt_writer_refs.preflight_ref
            == "work/module-receipt-preflights/pubpunk-publish-community-lab.md"
        && receipt_writer_refs.policy_gate_preflight_ref == policy_gate_preflight.preflight_id
        && receipt_writer_refs.receipt_target_ref == side_effect_refs.receipt_proposal_ref
        && receipt_writer_refs.storage_ref == ".punk/runs/pubpunk-publish-request/receipts"
        && receipt_writer_refs.channel_ref == side_effect_refs.target_ref
        && receipt_writer_refs.connector_profile_resolution_ref
            == side_effect_refs.connector_profile_resolution_ref
        && receipt_writer_refs.connector_profile_ref == side_effect_refs.connector_profile_ref
        && receipt_writer_refs.selected_connector_strategy_ref
            == side_effect_refs.selected_connector_strategy_ref
        && receipt_writer_refs.adapter_invocation_receipt_ref
            == policy_gate_preflight.adapter_invocation_receipt_ref
        && receipt_writer_refs.payload_ref == side_effect_refs.payload_ref
        && receipt_writer_preflight.status == ModuleHostStatus::Ready
        && receipt_writer_preflight.kind == ModuleSideEffectKind::Publish
        && receipt_writer_preflight.preflight_id
            == "work/module-receipt-preflights/pubpunk-publish-community-lab.md"
        && receipt_writer_preflight.policy_gate_preflight_ref == policy_gate_preflight.preflight_id
        && receipt_writer_preflight.receipt_target_ref == side_effect_refs.receipt_proposal_ref
        && receipt_writer_preflight.storage_ref == ".punk/runs/pubpunk-publish-request/receipts"
        ...
        && receipt_writer_preflight
            .covered_requirements
            .contains(&ModuleSideEffectReceiptWriterPreflightRequirement::PayloadRef)
        && receipt_writer_preflight
            .boundary_flags
            .all_side_effect_flags_false();
    ...
    if ok {
        SmokeEvalCaseResult::pass(
            "eval_pubpunk_publish_receipt_preflight_chains_to_host_receipt_writer_preflight",
            "PubPunk publish receipt preflight chains to host receipt writer preflight",
            "PubPunk publish receipt preflight consumed resolved connector refs and projected explicit receipt target, storage, operation-evidence, idempotency, rollback, error, adapter invocation receipt, connector, channel, and payload refs into existing Module Host side-effect receipt writer preflight without writing receipts, invoking adapters, publishing, reading credentials, or activating runtime",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_pubpunk_publish_receipt_preflight_chains_to_host_receipt_writer_preflight",
            "PubPunk publish receipt preflight chains to host receipt writer preflight",
            "PubPunk publish receipt preflight drifted from explicit no-execution receipt writer preflight boundary",
        )
    }
}

fn eval_pubpunk_publish_receipt_write_handoff_chains_to_host_receipt_writer_write(
) -> SmokeEvalCaseResult {
    let invocation = ModuleInvocationEnvelope::new(
        "pubpunk",
        "v0.1",
    ...
    let handoff_packet = PubPunkPublishReceiptWriteHandoffPacket::new(
        "v0.1",
        "contracts/pubpunk-publish-cycle-0",
        "runs/pubpunk-publish-request",
        "project/punk",
    ...
    .with_receipt_writer_active_behavior_ref(
        "work/module-receipt-writer-active-behavior/pubpunk-publish-community-lab.md",
    )
    .with_receipt_writer_file_io_plan_ref(
        "work/module-receipt-writer-file-io/pubpunk-publish-community-lab.md",
    )
    .with_receipt_writer_target_storage_policy_ref(
        "work/module-receipt-writer-target-storage-policy/pubpunk-publish-community-lab.md",
    )
    .with_receipt_writer_host_path_observation_ref(
        "work/module-receipt-writer-host-path-observation/pubpunk-publish-community-lab.md",
    )
    .with_receipt_writer_concrete_path_storage_policy_ref(
        "work/module-receipt-writer-concrete-path-storage-policy/pubpunk-publish-community-lab.md",
    )
    .with_operation_evidence_persistence_ref(
        "work/module-receipt-writer-operation-evidence-persistence/pubpunk-publish-community-lab.md",
    )
    .with_receipt_target_ref("work/module-receipts/pubpunk-publish-community-lab.md")
    .with_receipt_storage_ref(".punk/runs/pubpunk-publish-request")
    .with_receipt_target_path_ref(".punk/runs/pubpunk-publish-request/receipt.json")
    .with_receipt_bytes_ref("work/module-receipt-bytes/pubpunk-publish-community-lab.json")
    .with_operation_evidence_ref(
        ".punk/runs/pubpunk-publish-request/operation-evidence/pubpunk-publish-community-lab.md",
    )
    .with_idempotency_ref("work/module-idempotency/pubpunk-publish-community-lab.md")
    ...
    let handoff_assessment = assess_pubpunk_publish_receipt_write_handoff_packet(&handoff_packet);
    let handoff_refs = match handoff_packet.try_into_receipt_write_handoff_refs() {
        Ok(refs) => refs,
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_publish_receipt_write_handoff_chains_to_host_receipt_writer_write",
                "PubPunk publish receipt write handoff chains to host receipt writer write",
                "ready PubPunk publish receipt write handoff packet failed to build host writer refs",
            );
        }
    };

    let receipt_writer_draft = ModuleSideEffectReceiptWriterPreflightDraft::new(
        handoff_refs.receipt_writer_preflight_ref.clone(),
    )
    .with_policy_gate_preflight_ref(policy_gate_preflight.preflight_id.clone())
    .with_receipt_target_ref(handoff_refs.receipt_target_ref.clone())
    .with_storage_ref(handoff_refs.storage_ref.clone())
    .with_operation_evidence_ref(handoff_refs.operation_evidence_ref.clone())
    .with_idempotency_ref(handoff_refs.idempotency_ref.clone())
    .with_rollback_ref(handoff_refs.rollback_ref.clone())
    .with_error_ref(handoff_refs.error_ref.clone())
    .with_adapter_invocation_receipt_ref(handoff_refs.adapter_invocation_receipt_ref.clone())
    .with_payload_ref(handoff_refs.payload_ref.clone());
    let receipt_writer_preflight =
        preflight_module_side_effect_receipt_writer(&policy_gate_preflight, &receipt_writer_draft);
    ...
    let ready_plan = plan_module_side_effect_receipt_writer_file_io(
        &planned_behavior,
        handoff_refs.receipt_target_path_ref.clone(),
        ...
        vec!["PubPunk handoff selects explicit local receipt write target only.".to_owned()],
    );
    ...
    let ready_target_storage_policy = model_module_side_effect_receipt_writer_target_storage_policy(
        &ready_plan,
        ready_policy_refs,
        vec!["PubPunk handoff uses selected host target/storage policies.".to_owned()],
    );
    let ready_observation = model_module_side_effect_receipt_writer_host_path_observation(
        ...
        Some("runtime-relative:pubpunk-publish-request/receipt.json".to_owned()),
        true,
        vec![],
        vec!["PubPunk handoff host path observation remains redacted.".to_owned()],
    );
    let ready_concrete_policy =
        model_module_side_effect_receipt_writer_concrete_path_storage_policy(
            &ready_target_storage_policy,
            &ready_observation,
            vec!["PubPunk handoff concrete path/storage policy is ready.".to_owned()],
        );
    let ready_persistence = model_module_side_effect_receipt_writer_operation_evidence_persistence(
        &ready_concrete_policy,
        vec!["PubPunk handoff consumes ready operation evidence persistence policy.".to_owned()],
    );

    let storage_root = unique_smoke_temp_path();
    let target_relative = std::path::Path::new(handoff_refs.receipt_target_path_ref.as_str());
    let target_path = storage_root.join(target_relative);
    let setup_ok =
        fs::create_dir_all(storage_root.join(".punk/runs/pubpunk-publish-request")).is_ok();
    let receipt_bytes = br#"{"schema_version":"punk.module_host.side_effect_receipt.v0.1","receipt_id":"pubpunk-write-handoff-smoke-001"}"#;

    let write_result = write_module_side_effect_receipt_first_active_slice(
        &ready_persistence,
        receipt_bytes,
        &storage_root,
        target_relative,
        vec!["PubPunk handoff smoke writes exact receipt bytes to an explicit temporary .punk/runs target only.".to_owned()],
    );
    ...
    let forbidden_runtime_exists = storage_root.join(".punk/events").exists()
        || storage_root.join(".punk/decisions").exists()
        || storage_root.join(".punk/proofs").exists();
    let cleanup_ok = fs::remove_dir_all(&storage_root).is_ok();

    let ok = invocation_preflight.status == ModuleHostStatus::Ready
        && envelope.status == ModuleHostStatus::Ready
        && receipt_proposal.status == ModuleHostStatus::Ready
        && side_effect_proposal.status == ModuleHostStatus::Ready
        && policy_gate_preflight.status == ModuleHostStatus::Ready
        && handoff_assessment.status == PubPunkAssessmentStatus::Ready
        && handoff_assessment.authority == PubPunkAssessmentAuthority::Advisory
        && !handoff_assessment.has_blockers()
        && handoff_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && handoff_assessment.refs.receipt_target_path_ref
            == ".punk/runs/pubpunk-publish-request/receipt.json"
        && handoff_assessment.refs.receipt_bytes_ref
            == "work/module-receipt-bytes/pubpunk-publish-community-lab.json"
        && handoff_assessment.refs.connector_profile_resolution_ref
            == "work/module-assessments/pubpunk-connector-profile-resolution.md"
        && handoff_assessment.refs.connector_profile_ref
            == "publishing/connectors/github-discussions-community-lab.md"
        && handoff_assessment.refs.selected_connector_strategy_ref
            == "work/module-assessments/pubpunk-selected-connector-strategy.md"
        && handoff_refs.operation_evidence_persistence_ref
            == "work/module-receipt-writer-operation-evidence-persistence/pubpunk-publish-community-lab.md"
        && handoff_refs.channel_ref == "publishing/channels/github-discussions-community-lab.md"
        ...
        && setup_ok
        && write_result.schema_version
            == MODULE_HOST_SIDE_EFFECT_RECEIPT_WRITER_FIRST_ACTIVE_WRITE_SLICE_SCHEMA_VERSION
        && write_result.status == ModuleHostStatus::Ready
        && write_result.outcome == ModuleSideEffectReceiptWriterOutcome::Written
        && write_result.represents_new_receipt_write()
        && write_result.is_success()
        && write_result.blockers.is_empty()
        && matches!(&written, Ok(bytes) if bytes.as_slice() == receipt_bytes)
        && write_result.receipt_byte_len == receipt_bytes.len()
        ...
        && write_result.touches_filesystem()
        && !write_result.persists_operation_evidence()
        && !write_result.writes_event_log()
        && !write_result.invokes_adapter()
        && !write_result.invokes_policy_engine()
        && !write_result.invokes_gate()
        && !write_result.publishes()
        && !write_result.comments()
        && !write_result.creates_pull_request()
        && !write_result.writes_gate_decision()
    ...
    if ok {
        SmokeEvalCaseResult::pass(
            "eval_pubpunk_publish_receipt_write_handoff_chains_to_host_receipt_writer_write",
            "PubPunk publish receipt write handoff chains to host receipt writer write",
            "PubPunk publish receipt write handoff consumed resolved connector refs and projected explicit preflight, policy, path, receipt-bytes, and evidence refs into the existing Module Host receipt writer, which wrote exact bytes only to a temporary .punk/runs target without invoking adapters, publishing, mutating event logs, writing gate/proof authority, or claiming acceptance",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_pubpunk_publish_receipt_write_handoff_chains_to_host_receipt_writer_write",
            "PubPunk publish receipt write handoff chains to host receipt writer write",
            format!(
                "PubPunk receipt write handoff drifted; handoff_status={:?} writer_status={:?} persistence_ready={} write_outcome={:?} blockers={:?} written={:?} setup_ok={setup_ok} cleanup_ok={cleanup_ok}",
                handoff_assessment.status,
                receipt_writer_preflight.status,
                ready_persistence.is_ready(),
...
fn eval_pubpunk_publish_operation_evidence_handoff_chains_to_host_operation_evidence_write(
) -> SmokeEvalCaseResult {
    let invocation = ModuleInvocationEnvelope::new(
        "pubpunk",
        "v0.1",
    ...
    let receipt_handoff_packet = PubPunkPublishReceiptWriteHandoffPacket::new(
        "v0.1",
        "contracts/pubpunk-publish-cycle-0",
        "runs/pubpunk-publish-request",
        "project/punk",
    ...
    .with_receipt_target_path_ref(".punk/runs/pubpunk-publish-request/receipt.json")
    .with_receipt_bytes_ref("work/module-receipt-bytes/pubpunk-publish-community-lab.json")
    .with_operation_evidence_ref(".punk/runs/pubpunk-publish-request/operation-evidence.jsonl")
    .with_idempotency_ref("work/module-idempotency/pubpunk-publish-community-lab.md")
    .with_rollback_ref("work/module-rollbacks/pubpunk-publish-community-lab.md")
    .with_error_ref("work/module-errors/pubpunk-publish-community-lab.md")
    .with_adapter_invocation_receipt_ref(
    ...
    .with_token_cost_ref("work/reports/pubpunk-publish-receipt-write-handoff-token-cost.md");
    let receipt_handoff_assessment =
        assess_pubpunk_publish_receipt_write_handoff_packet(&receipt_handoff_packet);
    let receipt_handoff_refs = match receipt_handoff_packet.try_into_receipt_write_handoff_refs() {
        Ok(refs) => refs,
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_publish_operation_evidence_handoff_chains_to_host_operation_evidence_write",
    ...
    let operation_handoff_packet = PubPunkPublishOperationEvidenceHandoffPacket::new(
        "v0.1",
        "contracts/pubpunk-publish-cycle-0",
        "runs/pubpunk-publish-request",
        "project/punk",
    ...
    .with_receipt_storage_ref(receipt_handoff_refs.storage_ref.clone())
    .with_receipt_target_ref(receipt_handoff_refs.receipt_target_ref.clone())
    .with_receipt_target_path_ref(receipt_handoff_refs.receipt_target_path_ref.clone())
    .with_receipt_bytes_ref(receipt_handoff_refs.receipt_bytes_ref.clone())
    .with_operation_evidence_ref(receipt_handoff_refs.operation_evidence_ref.clone())
    .with_operation_evidence_target_path_ref(receipt_handoff_refs.operation_evidence_ref.clone())
    ...
    .with_operation_evidence_write_result_ref(
        "work/module-operation-evidence-results/pubpunk-publish-community-lab.md",
    )
    .with_idempotency_ref(receipt_handoff_refs.idempotency_ref.clone())
    .with_rollback_ref(receipt_handoff_refs.rollback_ref.clone())
    .with_error_ref(receipt_handoff_refs.error_ref.clone())
    .with_adapter_invocation_receipt_ref(
    ...
    .with_token_cost_ref("work/reports/pubpunk-publish-operation-evidence-handoff-token-cost.md");
    let operation_handoff_assessment =
        assess_pubpunk_publish_operation_evidence_handoff_packet(&operation_handoff_packet);
    let operation_handoff_refs = match operation_handoff_packet
        .try_into_operation_evidence_handoff_refs()
    {
        ...
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                    "eval_pubpunk_publish_operation_evidence_handoff_chains_to_host_operation_evidence_write",
                    "PubPunk publish operation evidence handoff chains to host operation evidence write",
                    "ready PubPunk publish operation evidence handoff packet failed to build host writer refs",
                );
        }
    };

    let receipt_writer_draft = ModuleSideEffectReceiptWriterPreflightDraft::new(
        receipt_handoff_refs.receipt_writer_preflight_ref.clone(),
    )
    .with_policy_gate_preflight_ref(policy_gate_preflight.preflight_id.clone())
    .with_receipt_target_ref(receipt_handoff_refs.receipt_target_ref.clone())
    .with_storage_ref(receipt_handoff_refs.storage_ref.clone())
    .with_operation_evidence_ref(receipt_handoff_refs.operation_evidence_ref.clone())
    .with_idempotency_ref(receipt_handoff_refs.idempotency_ref.clone())
    .with_rollback_ref(receipt_handoff_refs.rollback_ref.clone())
    .with_error_ref(receipt_handoff_refs.error_ref.clone())
    ...
    let ready_plan = plan_module_side_effect_receipt_writer_file_io(
        &planned_behavior,
        receipt_handoff_refs.receipt_target_path_ref.clone(),
        ModuleSideEffectReceiptWriterWritePolicy::IdempotentIfMatching,
        ModuleSideEffectReceiptWriterIdempotencyBasis::ReceiptTargetAndPayloadRefs,
        ModuleSideEffectReceiptWriterTempAtomicPolicy::ExplicitNonAtomic,
        vec![
        ...
        vec![
            "PubPunk operation evidence handoff selects explicit local receipt target only."
                .to_owned(),
        ],
    );
    let ready_policy_refs = ModuleSideEffectReceiptWriterTargetStoragePolicyRefs::all_selected(
    ...
    let ready_target_storage_policy = model_module_side_effect_receipt_writer_target_storage_policy(
        ...
        ready_policy_refs,
        vec![
            "PubPunk operation evidence handoff uses selected host target/storage policies."
                .to_owned(),
        ],
    );
    let ready_observation = model_module_side_effect_receipt_writer_host_path_observation(
        ...
        vec![],
        vec![
            "PubPunk operation evidence handoff host path observation remains redacted.".to_owned(),
        ],
    );
    let ready_concrete_policy =
        model_module_side_effect_receipt_writer_concrete_path_storage_policy(
            ...
            &ready_observation,
            vec![
                "PubPunk operation evidence handoff concrete path/storage policy is ready."
                    .to_owned(),
            ],
        );
    let ready_persistence = model_module_side_effect_receipt_writer_operation_evidence_persistence(
        &ready_concrete_policy,
        vec![
            "PubPunk operation evidence handoff consumes ready operation evidence policy."
                .to_owned(),
        ],
    );

    let storage_root = unique_smoke_temp_path();
    let run_dir = storage_root.join(".punk/runs/pubpunk-publish-request");
    let receipt_relative =
        std::path::Path::new(receipt_handoff_refs.receipt_target_path_ref.as_str());
    let evidence_relative = std::path::Path::new(
        operation_handoff_refs
            .operation_evidence_target_path_ref
            .as_str(),
    );
    let evidence_path = storage_root.join(evidence_relative);
    let setup_ok = fs::create_dir_all(&run_dir).is_ok();
    let receipt_bytes = br#"{"schema_version":"punk.module_host.side_effect_receipt.v0.1","receipt_id":"pubpunk-operation-evidence-handoff-smoke-001"}"#;
    let operation_evidence_bytes = br#"{"schema_version":"punk.module_host.side_effect_receipt_writer.operation_evidence.v0.1","receipt_id":"pubpunk-operation-evidence-handoff-smoke-001","operation_evidence_authority":"advisory"}"#;

    let receipt_result = write_module_side_effect_receipt_first_active_slice(
        &ready_persistence,
        receipt_bytes,
        &storage_root,
        receipt_relative,
        vec!["PubPunk operation evidence handoff smoke first writes exact receipt bytes to an explicit temporary .punk/runs target only.".to_owned()],
    );
    let evidence_result =
        write_module_side_effect_receipt_writer_operation_evidence_first_active_slice(
            &receipt_result,
            ...
            evidence_relative,
            vec![
                "PubPunk operation evidence handoff smoke writes exact evidence bytes to an explicit temporary .punk/runs target only."
                    .to_owned(),
            ],
        );
    let written = fs::read(&evidence_path);
    let idempotent = write_module_side_effect_receipt_writer_operation_evidence_first_active_slice(
        &receipt_result,
        operation_evidence_bytes,
        &storage_root,
        evidence_relative,
    ...
    let ok = invocation_preflight.status == ModuleHostStatus::Ready
        ...
        && side_effect_proposal.status == ModuleHostStatus::Ready
        && policy_gate_preflight.status == ModuleHostStatus::Ready
        && receipt_handoff_assessment.status == PubPunkAssessmentStatus::Ready
        && operation_handoff_assessment.status == PubPunkAssessmentStatus::Ready
        && operation_handoff_assessment.authority == PubPunkAssessmentAuthority::Advisory
        && !operation_handoff_assessment.has_blockers()
        && operation_handoff_assessment
            .boundary_flags
            .all_side_effect_flags_false()
        && operation_handoff_assessment
            .refs
            .publish_receipt_write_handoff_ref
            == "work/module-assessments/pubpunk-publish-receipt-write-handoff.md"
        && operation_handoff_assessment
            .refs
            .operation_evidence_target_path_ref
        ...
        && ready_persistence.is_ready()
        && setup_ok
        && receipt_result.is_success()
        && receipt_result.outcome == ModuleSideEffectReceiptWriterOutcome::Written
        && !receipt_result.persists_operation_evidence()
        && evidence_result.schema_version
            == MODULE_HOST_SIDE_EFFECT_RECEIPT_WRITER_OPERATION_EVIDENCE_WRITE_SLICE_SCHEMA_VERSION
        && evidence_result.status == ModuleHostStatus::Ready
        && evidence_result.operation_evidence_outcome
            == ModuleSideEffectReceiptWriterOutcome::Written
        && evidence_result.represents_new_operation_evidence_write()
        && evidence_result.is_success()
        && evidence_result.blockers.is_empty()
        && evidence_result.blockers_fail_closed()
        && matches!(&written, Ok(bytes) if bytes.as_slice() == operation_evidence_bytes)
        && evidence_result.operation_evidence_byte_len == operation_evidence_bytes.len()
        && idempotent.operation_evidence_outcome
        ...
        && evidence_result.touches_filesystem()
        && evidence_result.persists_operation_evidence()
        && !evidence_result.writes_event_log()
        && !evidence_result.invokes_adapter()
        && !evidence_result.invokes_policy_engine()
        && !evidence_result.invokes_gate()
        && !evidence_result.publishes()
        && !evidence_result.comments()
        && !evidence_result.creates_pull_request()
        && !evidence_result.writes_gate_decision()
    ...
    if ok {
        ...
        SmokeEvalCaseResult::fail(
            ...
            "PubPunk publish operation evidence handoff chains to host operation evidence write",
            format!(
                "PubPunk operation evidence handoff drifted; receipt_handoff_status={:?} operation_handoff_status={:?} writer_status={:?} persistence_ready={} receipt_outcome={:?} evidence_outcome={:?} evidence_blockers={:?} written={:?} setup_ok={setup_ok} cleanup_ok={cleanup_ok}",
                receipt_handoff_assessment.status,
                operation_handoff_assessment.status,
                receipt_writer_preflight.status,
                ready_persistence.is_ready(),
                receipt_result.outcome,
                evidence_result.operation_evidence_outcome,
...
fn eval_pubpunk_publish_receipt_evidence_event_handoff_chains_to_local_event_log(
) -> SmokeEvalCaseResult {
    let operation_handoff_packet = PubPunkPublishOperationEvidenceHandoffPacket::new(
        "v0.1",
        "contracts/pubpunk-publish-cycle-0",
        ...
        .try_into_operation_evidence_handoff_refs()
    {
        ...
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_publish_receipt_evidence_event_handoff_chains_to_local_event_log",
                "PubPunk publish receipt/evidence event handoff chains to local event log",
                "ready PubPunk publish operation evidence handoff packet failed to build resolved connector refs",
            );
        }
    };

    let packet = PubPunkPublishReceiptEvidenceEventHandoffPacket::new(
        "v0.1",
        "contracts/pubpunk-publish-cycle-0",
        "runs/pubpunk-publish-request",
        "project/punk",
    ...
    .with_receipt_writer_result_ref(operation_handoff_refs.receipt_writer_result_ref.clone())
    .with_operation_evidence_write_result_ref(
        operation_handoff_refs
            .operation_evidence_write_result_ref
            .clone(),
    )
    .with_receipt_ref(operation_handoff_refs.receipt_target_path_ref.clone())
    .with_operation_evidence_ref(
        operation_handoff_refs
            .operation_evidence_target_path_ref
            .clone(),
    ...
    .with_payload_ref(operation_handoff_refs.payload_ref.clone())
    .with_channel_ref(operation_handoff_refs.channel_ref.clone())
    .with_connector_profile_resolution_ref(
        operation_handoff_refs
            .connector_profile_resolution_ref
            .clone(),
    )
    .with_connector_profile_ref(operation_handoff_refs.connector_profile_ref.clone())
    .with_selected_connector_strategy_ref(
        operation_handoff_refs
            .selected_connector_strategy_ref
            .clone(),
    ...
    let refs = match packet.try_into_receipt_evidence_event_handoff_refs() {
        Ok(refs) => refs,
        Err(_) => {
            return SmokeEvalCaseResult::fail(
                "eval_pubpunk_publish_receipt_evidence_event_handoff_chains_to_local_event_log",
                "PubPunk publish receipt/evidence event handoff chains to local event log",
                "ready PubPunk publish receipt/evidence event handoff packet failed to build event refs",
            );
        }
    };
    ...
    let append_result = append_local_receipt_evidence_handoff_event(
        &temp_path,
        EventSource::new("pubpunk", "receipt_evidence_event_handoff"),
        EventCorrelation::new("flow_pubpunk_publish_request").with_goal_ref(
            "work/goals/goal_add_pubpunk_publish_receipt_evidence_event_handoff_v0_1.md",
        ),
        refs.receipt_ref.as_str(),
        refs.operation_evidence_ref.as_str(),
    );
    let rendered = fs::read_to_string(temp_path.join(".punk/events/flow.jsonl"));
    let forbidden_runtime_created = temp_path.join(".punk/runs").exists()
        || temp_path.join(".punk/decisions").exists()
    ...
    let result = match (append_result, rendered) {
        (Ok(record), Ok(text))
            if assessment.status == PubPunkAssessmentStatus::Ready
                && operation_handoff_assessment.status == PubPunkAssessmentStatus::Ready
                && assessment.authority == PubPunkAssessmentAuthority::Advisory
                && !assessment.has_blockers()
                && assessment.boundary_flags.all_side_effect_flags_false()
                && assessment.refs.event_log_ref == ".punk/events/flow.jsonl"
                ...
                && assessment.refs.selected_connector_strategy_ref
                    == operation_handoff_refs.selected_connector_strategy_ref
                && refs.event_log_ref == ".punk/events/flow.jsonl"
                && refs.connector_profile_resolution_ref
                    == operation_handoff_refs.connector_profile_resolution_ref
                && refs.connector_profile_ref == operation_handoff_refs.connector_profile_ref
                && refs.selected_connector_strategy_ref
                    == operation_handoff_refs.selected_connector_strategy_ref
                && record.sequence == 1
                && record.event_id == "evt_0000000000000001"
                && record.kind == EventKind::ReceiptEvidenceHandoff
                ...
                && record.artifacts.refs
                    == vec![refs.receipt_ref.clone(), refs.operation_evidence_ref.clone()]
                && text.lines().count() == 1
                && text.contains("\"kind\":\"receipt_evidence_handoff\"")
                && text.contains(refs.receipt_ref.as_str())
                && text.contains(refs.operation_evidence_ref.as_str())
                && !text.contains("decision_id")
                && !forbidden_runtime_created =>
        {
            SmokeEvalCaseResult::pass(
                "eval_pubpunk_publish_receipt_evidence_event_handoff_chains_to_local_event_log",
                "PubPunk publish receipt/evidence event handoff chains to local event log",
                "PubPunk publish receipt/evidence event handoff consumed resolved connector refs before projecting explicit receipt, operation-evidence, writer-result, event source, event correlation, channel, connector, and payload refs into the existing local receipt/evidence event writer, which appended one bounded event under a temporary .punk/events log without writing .punk/runs artifacts, invoking adapters, publishing, writing gate/proof authority, or claiming acceptance",
            )
        }
        (Ok(record), Ok(text)) => SmokeEvalCaseResult::fail(
            "eval_pubpunk_publish_receipt_evidence_event_handoff_chains_to_local_event_log",
            "PubPunk publish receipt/evidence event handoff chains to local event log",
            format!(
                "PubPunk receipt/evidence event handoff drifted; packet_status={:?} event_id={} sequence={} kind={} lines={} forbidden_runtime_created={}",
                assessment.status,
                record.event_id,
                record.sequence,
                record.kind.as_str(),
        ...
        (Err(error), _) => SmokeEvalCaseResult::fail(
            "eval_pubpunk_publish_receipt_evidence_event_handoff_chains_to_local_event_log",
            "PubPunk publish receipt/evidence event handoff chains to local event log",
            format!("PubPunk receipt/evidence event append failed with {error:?}"),
        ),
        (_, Err(error)) => SmokeEvalCaseResult::fail(
            "eval_pubpunk_publish_receipt_evidence_event_handoff_chains_to_local_event_log",
            "PubPunk publish receipt/evidence event handoff chains to local event log",
            format!("PubPunk receipt/evidence event log read failed with {error:?}"),
        ),
    };
...
fn eval_module_host_side_effect_receipt_writer_file_io_plan_model_is_side_effect_free(
) -> SmokeEvalCaseResult {
    let invocation = ModuleInvocationEnvelope::new(
        "pubpunk",
        "v0.1",
    ...
    let incomplete_plan = plan_module_side_effect_receipt_writer_file_io(
        &planned_behavior,
        "",
    ...
    let non_planned_plan = plan_module_side_effect_receipt_writer_file_io(
        &written_behavior,
        "../receipt.json",
    ...
    let ok = receipt_writer_preflight.status == ModuleHostStatus::Ready
        && planned_behavior.status == ModuleHostStatus::Ready
        && ready_plan.status == ModuleHostStatus::Ready
        && ready_plan.file_io_status().as_str() == "ready"
        && ready_plan.is_file_io_ready()
        && ready_plan.selects_receipt_write()
        && ready_plan.tracks_conflict_visibility()
        && ready_plan.tracks_rollback_visibility()
        && ready_plan.tracks_operation_evidence_persistence_visibility()
        && ready_plan.write_policy.supports_idempotency()
        && !ready_plan.write_policy.allows_silent_overwrite()
        && ready_plan.temp_atomic_policy.prefers_atomic_move()
        && ready_plan.operation_outcome() == ModuleSideEffectReceiptWriterOutcome::PlannedOnly
        && ready_plan.boundary_flags.all_side_effect_flags_false()
        && ready_plan.is_evidence_only()
        && !ready_plan.touches_filesystem()
        && !ready_plan.writes_receipt()
        && !ready_plan.persists_operation_evidence()
        && !ready_plan.target_path_is_authority()
        && incomplete_plan.status == ModuleHostStatus::Blocked
        && incomplete_plan
            .blockers
            .contains(&ModuleSideEffectReceiptWriterFileIoPlanBlocker::MissingTargetPathRef)
        && incomplete_plan.blockers.contains(
        ...
        && non_planned_plan.blockers.contains(
    ...
    if ok {
        SmokeEvalCaseResult::pass(
            "eval_module_host_side_effect_receipt_writer_file_io_plan_model_is_side_effect_free",
            "module-host side-effect receipt writer file IO plan model stays local and side-effect-free",
            "module-host side-effect receipt writer file IO plan modeled storage refs, receipt target refs, target path refs, write policy, idempotency, temp/atomic policy, rollback/error visibility, and operation-evidence persistence visibility without writing receipts, mutating event logs, persisting operation evidence, reading or writing files, invoking adapters, invoking policy engines, invoking gate, publishing, creating pull requests, calling APIs, reading credentials, writing proofpacks, or claiming acceptance",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_module_host_side_effect_receipt_writer_file_io_plan_model_is_side_effect_free",
            "module-host side-effect receipt writer file IO plan model stays local and side-effect-free",
            "module-host side-effect receipt writer file IO plan drifted from pure no-IO advisory boundary",
        )
    }
}

fn eval_module_host_side_effect_receipt_writer_target_storage_policy_model_is_side_effect_free(
) -> SmokeEvalCaseResult {
    let invocation = ModuleInvocationEnvelope::new(
        "pubpunk",
        "v0.1",
    ...
    let ready_model = model_module_side_effect_receipt_writer_target_storage_policy(
        &ready_plan,
        ready_policy_refs,
        vec!["Target/storage policy evidence does not authorize file IO.".to_owned()],
    );
    let missing_policy_model = model_module_side_effect_receipt_writer_target_storage_policy(
        &ready_plan,
    ...
    let unsafe_plan = plan_module_side_effect_receipt_writer_file_io(
        &planned_behavior,
        ".punk/runs/pubpunk-publish-community-lab/../receipt.json",
        ModuleSideEffectReceiptWriterWritePolicy::IdempotentIfMatching,
        ModuleSideEffectReceiptWriterIdempotencyBasis::ReceiptTargetAndPayloadRefs,
        ...
        vec!["Unsafe target path stays model data only.".to_owned()],
    );
    let unsafe_policy_model = model_module_side_effect_receipt_writer_target_storage_policy(
        &unsafe_plan,
        ModuleSideEffectReceiptWriterTargetStoragePolicyRefs::new(
        ...
        vec!["Unsafe policy refs fail closed before any IO.".to_owned()],
    );

    let ok = receipt_writer_preflight.status == ModuleHostStatus::Ready
        && ready_plan.is_file_io_ready()
        && ready_model.is_ready()
        && ready_model.policy_status.as_str() == "ready"
        && ready_model.policy_refs.all_required_selected()
        && ready_model.refs_are_separated()
        && ready_model.blockers.is_empty()
        && ready_model.blockers_fail_closed()
        && ready_model.operation_outcome() == ModuleSideEffectReceiptWriterOutcome::PlannedOnly
        && ready_model.boundary_flags.models_target_storage_policy
        && ready_model.boundary_flags.requires_ready_file_io_plan
        && ready_model.boundary_flags.requires_selected_policy_refs
        && ready_model.boundary_flags.all_side_effect_flags_false()
        && ready_model.is_evidence_only()
        && ready_model.policy_refs_are_operational_evidence()
        && !ready_model.touches_filesystem()
        && !ready_model.writes_receipt()
        && !ready_model.persists_operation_evidence()
        && !ready_model.invokes_adapter()
        && !ready_model.target_path_ref_is_authority()
        && !ready_model.policy_refs_are_authority()
        && missing_policy_model.is_blocked()
        && missing_policy_model.has_blocker(
        ...
        && unsafe_policy_model.is_blocked()
        && unsafe_policy_model.has_blocker(
        ...
        && unsafe_policy_model.blockers_fail_closed()
        && !unsafe_policy_model.touches_filesystem();

    if ok {
        SmokeEvalCaseResult::pass(
            "eval_module_host_side_effect_receipt_writer_target_storage_policy_model_is_side_effect_free",
            "module-host side-effect receipt writer target/storage policy model stays local and side-effect-free",
            "module-host side-effect receipt writer target/storage policy modeled storage root, receipt target, target path, policy refs, failure visibility, and fail-closed blockers without writing receipts, mutating event logs, persisting operation evidence, reading or writing files, invoking adapters, invoking policy engines, invoking gate, publishing, creating pull requests, calling APIs, reading credentials, writing proofpacks, or claiming acceptance",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_module_host_side_effect_receipt_writer_target_storage_policy_model_is_side_effect_free",
            "module-host side-effect receipt writer target/storage policy model stays local and side-effect-free",
            "module-host side-effect receipt writer target/storage policy drifted from pure no-IO advisory boundary",
        )
    }
}

fn eval_module_host_side_effect_receipt_writer_host_path_observation_model_is_side_effect_free(
) -> SmokeEvalCaseResult {
    let invocation = ModuleInvocationEnvelope::new(
        "pubpunk",
        "v0.1",
    ...
    let blocked_from_policy = model_module_side_effect_receipt_writer_host_path_observation(
        &missing_policy_model,
    ...
    let ok = receipt_writer_preflight.status == ModuleHostStatus::Ready
        && ready_target_storage_policy.is_ready()
        && ready_observation.status == ModuleHostStatus::Ready
        && ready_observation.is_observed()
        && ready_observation.observation_status.as_str() == "observed"
        && ready_observation.refs_are_separated()
        && ready_observation.host_path_kind.as_str() == "storage_root_relative"
        && ready_observation.host_path_ref.as_deref()
            == Some("storage-root-relative:pubpunk-publish-community-lab/receipt.json")
        && ready_observation.host_path_redacted
        && ready_observation.blockers.is_empty()
        && ready_observation.blockers_fail_closed()
        && ready_observation.operation_outcome()
            == ModuleSideEffectReceiptWriterOutcome::PlannedOnly
        && ready_observation
            .boundary_flags
            .models_host_path_observation
        && ready_observation
            .boundary_flags
            .requires_ready_target_storage_policy
        && ready_observation
            .boundary_flags
            .requires_selected_policy_refs
        && ready_observation
            .boundary_flags
        ...
        && ready_observation
            .boundary_flags
            .all_side_effect_flags_false()
        && ready_observation.is_evidence_only()
        && !ready_observation.touches_filesystem()
        && !ready_observation.resolves_host_paths()
        && !ready_observation.canonicalizes_host_paths()
        && !ready_observation.writes_receipt()
        && !ready_observation.persists_operation_evidence()
        && !ready_observation.invokes_adapter()
        && !ready_observation.host_path_ref_is_authority()
        && !ready_observation.implies_receipt_availability()
        && blocked_observation.is_blocked()
        && blocked_observation.observation_status.as_str() == "blocked"
        && blocked_observation.has_blocker(
        ...
        && blocked_observation.blockers_fail_closed()
        && !blocked_observation.touches_filesystem()
        && !blocked_observation.writes_receipt()
        && !blocked_observation.persists_operation_evidence()
    ...
    if ok {
        SmokeEvalCaseResult::pass(
            "eval_module_host_side_effect_receipt_writer_host_path_observation_model_is_side_effect_free",
            "module-host side-effect receipt writer host path observation model stays local and side-effect-free",
            "module-host side-effect receipt writer host path observation modeled redacted host path observations and fail-closed blockers without writing receipts, mutating event logs, persisting operation evidence, reading or writing files, resolving or canonicalizing host paths, invoking adapters, invoking policy engines, invoking gate, publishing, creating pull requests, calling APIs, reading credentials, writing proofpacks, or claiming acceptance",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_module_host_side_effect_receipt_writer_host_path_observation_model_is_side_effect_free",
            "module-host side-effect receipt writer host path observation model stays local and side-effect-free",
            "module-host side-effect receipt writer host path observation drifted from pure no-IO advisory boundary",
        )
    }
}

fn eval_module_host_side_effect_receipt_writer_concrete_path_storage_policy_model_is_side_effect_free(
) -> SmokeEvalCaseResult {
    let invocation = ModuleInvocationEnvelope::new(
        "pubpunk",
        "v0.1",
    ...
    let blocked_concrete_policy_from_path =
        model_module_side_effect_receipt_writer_concrete_path_storage_policy(
            &ready_target_storage_policy,
            &blocked_observation,
    ...
    let mut mismatched_observation = ready_observation;
    mismatched_observation.target_path_ref = "other/receipt.json".to_owned();
    let blocked_concrete_policy_from_refs =
        model_module_side_effect_receipt_writer_concrete_path_storage_policy(
            &missing_policy_model,
    ...
    let ok = receipt_writer_preflight.status == ModuleHostStatus::Ready
        && ready_concrete_policy.is_ready()
        && ready_concrete_policy.concrete_policy_status.as_str() == "ready"
        && ready_concrete_policy.refs_are_separated()
        && ready_concrete_policy.policy_refs.all_required_selected()
        && ready_concrete_policy.host_path_kind.as_str() == "storage_root_relative"
        && ready_concrete_policy.host_path_ref.as_deref()
            == Some("storage-root-relative:pubpunk-publish-community-lab/receipt.json")
        && ready_concrete_policy.host_path_redacted
        && ready_concrete_policy.blockers.is_empty()
        && ready_concrete_policy.blockers_fail_closed()
        && ready_concrete_policy.operation_outcome()
            == ModuleSideEffectReceiptWriterOutcome::PlannedOnly
        && ready_concrete_policy
            .boundary_flags
            .models_concrete_path_storage_policy
        && ready_concrete_policy
            .boundary_flags
            .requires_ready_target_storage_policy
        && ready_concrete_policy
            .boundary_flags
            .requires_observed_host_path
        && ready_concrete_policy
            .boundary_flags
            .requires_selected_policy_refs
        && ready_concrete_policy
            .boundary_flags
            .requires_redacted_sensitive_host_path
        && ready_concrete_policy
            .boundary_flags
        ...
        && ready_concrete_policy
            .boundary_flags
            .all_side_effect_flags_false()
        && ready_concrete_policy.is_evidence_only()
        && !ready_concrete_policy.touches_filesystem()
        && !ready_concrete_policy.resolves_host_paths()
        && !ready_concrete_policy.canonicalizes_host_paths()
        && !ready_concrete_policy.writes_receipt()
        && !ready_concrete_policy.persists_operation_evidence()
        && !ready_concrete_policy.invokes_adapter()
        && !ready_concrete_policy.host_path_ref_is_authority()
        && !ready_concrete_policy.policy_refs_are_authority()
        && !ready_concrete_policy.implies_receipt_availability()
        && blocked_concrete_policy_from_path.is_blocked()
        && blocked_concrete_policy_from_path.concrete_policy_status.as_str() == "blocked"
        && blocked_concrete_policy_from_path.has_blocker(
        ...
        && blocked_concrete_policy_from_path.blockers_fail_closed()
        && !blocked_concrete_policy_from_path.touches_filesystem()
        && !blocked_concrete_policy_from_path.writes_receipt()
        && !blocked_concrete_policy_from_path.persists_operation_evidence()
        && blocked_concrete_policy_from_refs.has_blocker(
        ...
        && blocked_concrete_policy_from_refs.blockers_fail_closed()
        && !blocked_concrete_policy_from_refs.touches_filesystem()
        && !blocked_concrete_policy_from_refs.writes_receipt()
        && !blocked_concrete_policy_from_refs.persists_operation_evidence();

    if ok {
        SmokeEvalCaseResult::pass(
            "eval_module_host_side_effect_receipt_writer_concrete_path_storage_policy_model_is_side_effect_free",
            "module-host side-effect receipt writer concrete path/storage policy model stays local and side-effect-free",
            "module-host side-effect receipt writer concrete path/storage policy composed target/storage policy and host path observation readiness without writing receipts, mutating event logs, persisting operation evidence, reading or writing files, resolving or canonicalizing host paths, invoking adapters, invoking policy engines, invoking gate, publishing, creating pull requests, calling APIs, reading credentials, writing proofpacks, or claiming acceptance",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_module_host_side_effect_receipt_writer_concrete_path_storage_policy_model_is_side_effect_free",
...
fn eval_module_host_side_effect_receipt_writer_first_active_write_slice_writes_receipt_bytes(
) -> SmokeEvalCaseResult {
    let invocation = ModuleInvocationEnvelope::new(
        "pubpunk",
        "v0.1",
    ...
    let ready_plan = plan_module_side_effect_receipt_writer_file_io(
        ...
        vec!["First active receipt write smoke plan is explicit and local-only.".to_owned()],
    );
    let ready_policy_refs = ModuleSideEffectReceiptWriterTargetStoragePolicyRefs::all_selected(
        "policies/module-host/storage-root-selection.v0.1",
        "policies/module-host/receipt-target.v0.1",
    ...
    let ready_observation = model_module_side_effect_receipt_writer_host_path_observation(
        ...
        true,
        vec![],
        vec!["First active receipt write host path observation remains redacted.".to_owned()],
    );
    let ready_concrete_policy =
        model_module_side_effect_receipt_writer_concrete_path_storage_policy(
            &ready_target_storage_policy,
    ...
    let ready_persistence = model_module_side_effect_receipt_writer_operation_evidence_persistence(
        &ready_concrete_policy,
        vec!["First active receipt write consumes ready operation evidence policy.".to_owned()],
    );
    ...
    let target_path = storage_root.join(target_relative);
    let setup_ok =
        fs::create_dir_all(storage_root.join(".punk/runs/pubpunk-publish-community-lab")).is_ok();
    let receipt_bytes = br#"{"schema_version":"punk.module_host.side_effect_receipt.v0.1","receipt_id":"module-receipt-smoke-001"}"#;
    ...
    let write_result = write_module_side_effect_receipt_first_active_slice(
        ...
        &storage_root,
        target_relative,
        vec!["First active smoke writes exact receipt bytes to an explicit temporary .punk/runs target only.".to_owned()],
    );
    let written = fs::read(&target_path);
    let idempotent = write_module_side_effect_receipt_first_active_slice(
        &ready_persistence,
    ...
    let blocked_outside_punk_runs = write_module_side_effect_receipt_first_active_slice(
        &ready_persistence,
        receipt_bytes,
        &storage_root,
    ...
    let vocabulary_ok = MODULE_HOST_SIDE_EFFECT_RECEIPT_WRITER_FIRST_ACTIVE_WRITE_SLICE_SCHEMA_VERSION
        == "punk.module_host.side_effect_receipt_writer_first_active_write_slice.v0.1"
        && ModuleSideEffectReceiptWriterFirstActiveWriteSliceBlocker::OperationEvidencePersistenceNotReady.as_str()
            == "operation_evidence_persistence_not_ready"
        && ModuleSideEffectReceiptWriterFirstActiveWriteSliceBlocker::TargetPathOutsidePunkRuns.as_str()
            == "target_path_outside_punk_runs"
        && ModuleSideEffectReceiptWriterFirstActiveWriteSliceBlocker::ExistingTargetDifferent.as_str()
            == "existing_target_different";
    let write_ok = setup_ok
        && ready_persistence.is_ready()
        && write_result.schema_version
            == MODULE_HOST_SIDE_EFFECT_RECEIPT_WRITER_FIRST_ACTIVE_WRITE_SLICE_SCHEMA_VERSION
        && write_result.status == ModuleHostStatus::Ready
        ...
        && write_result.is_success()
        && write_result.blockers.is_empty()
        && write_result.blockers_fail_closed()
        && matches!(&written, Ok(bytes) if bytes.as_slice() == receipt_bytes)
        && write_result.receipt_byte_len == receipt_bytes.len()
        && idempotent.outcome == ModuleSideEffectReceiptWriterOutcome::Idempotent
        && idempotent.is_idempotent_existing_match()
        && idempotent.is_success()
        && blocked_outside_punk_runs.has_blocker(
        ...
        && blocked_outside_punk_runs.blockers_fail_closed()
        && !storage_root.join("receipts/outside.json").exists();
    let boundary_ok = boundary.implements_first_active_write_slice
        && boundary.requires_ready_operation_evidence_persistence
        && boundary.requires_explicit_storage_root_path
        && boundary.requires_explicit_punk_runs_target_relative_path
        && boundary.writes_exact_receipt_bytes
        && boundary.uses_create_new_no_overwrite
        && boundary.reads_filesystem
        && boundary.touches_filesystem
        && boundary.creates_receipt
        && boundary.writes_receipt
        && boundary.writes_punk_runs
        && boundary.writes_files
        && !boundary.persists_operation_evidence
        && !boundary.writes_event_log
        && !boundary.invokes_adapter
        && !boundary.invokes_policy_engine
        && !boundary.invokes_gate
        && !boundary.calls_external_apis
        && !boundary.opens_browser
        && !boundary.reads_credentials
        && !boundary.publishes
        && !boundary.comments
        && !boundary.creates_pull_request
        ...
        && boundary.receipt_is_non_authoritative
    ...
    if vocabulary_ok && write_ok && boundary_ok && no_forbidden_side_effects {
        SmokeEvalCaseResult::pass(
            "eval_module_host_side_effect_receipt_writer_first_active_write_slice_writes_receipt_bytes",
            "module-host side-effect receipt writer first active write slice writes exact receipt bytes",
            "module-host side-effect receipt writer first active write slice wrote exact receipt bytes to an explicit temporary .punk/runs target while leaving operation evidence persistence, event logs, adapters, policy engines, gate, publishing, comments, pull requests, proofpacks, and acceptance inactive",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_module_host_side_effect_receipt_writer_first_active_write_slice_writes_receipt_bytes",
            "module-host side-effect receipt writer first active write slice writes exact receipt bytes",
            format!(
                "module-host receipt writer first active write slice drifted; vocabulary={vocabulary_ok} write={write_ok} boundary={boundary_ok} no_forbidden_side_effects={no_forbidden_side_effects} outcome={:?} blockers={:?} idempotent={:?} blocked={:?} written={:?} setup_ok={setup_ok} cleanup_ok={cleanup_ok}",
                write_result.outcome,
                write_result.blockers,
                idempotent.outcome,
                blocked_outside_punk_runs.blockers,
                written.as_ref().map(|bytes| bytes.len()),
            ),
        )
    }
...
fn eval_module_host_side_effect_receipt_writer_operation_evidence_write_slice_writes_evidence_bytes(
) -> SmokeEvalCaseResult {
    let invocation = ModuleInvocationEnvelope::new(
        "pubpunk",
        "v0.1",
    ...
    let ready_plan = plan_module_side_effect_receipt_writer_file_io(
        ...
        vec!["Operation evidence write smoke plan is explicit and local-only.".to_owned()],
    );
    let ready_policy_refs = ModuleSideEffectReceiptWriterTargetStoragePolicyRefs::all_selected(
        "policies/module-host/storage-root-selection.v0.1",
        "policies/module-host/receipt-target.v0.1",
    ...
    let ready_target_storage_policy = model_module_side_effect_receipt_writer_target_storage_policy(
        &ready_plan,
        ready_policy_refs,
        vec!["Operation evidence write target/storage policy is explicit.".to_owned()],
    );
    let ready_observation = model_module_side_effect_receipt_writer_host_path_observation(
        &ready_target_storage_policy,
        ModuleSideEffectReceiptWriterHostPathKind::RuntimeRelative,
        ...
        true,
        vec![],
        vec!["Operation evidence write host path observation remains redacted.".to_owned()],
    );
    let ready_concrete_policy =
        model_module_side_effect_receipt_writer_concrete_path_storage_policy(
            &ready_target_storage_policy,
            &ready_observation,
            vec!["Operation evidence write concrete path policy is ready.".to_owned()],
        );
    let ready_persistence = model_module_side_effect_receipt_writer_operation_evidence_persistence(
        &ready_concrete_policy,
        vec!["Operation evidence write consumes ready operation evidence policy.".to_owned()],
    );

    let storage_root = unique_smoke_temp_path();
    let run_dir = storage_root.join(".punk/runs/pubpunk-publish-community-lab");
    let receipt_relative =
        std::path::Path::new(".punk/runs/pubpunk-publish-community-lab/receipt.json");
    let evidence_relative =
        std::path::Path::new(".punk/runs/pubpunk-publish-community-lab/operation-evidence.jsonl");
    let evidence_path = storage_root.join(evidence_relative);
    let setup_ok = fs::create_dir_all(&run_dir).is_ok();
    let receipt_bytes = br#"{"schema_version":"punk.module_host.side_effect_receipt.v0.1","receipt_id":"module-receipt-smoke-002"}"#;
    let operation_evidence_bytes = br#"{"schema_version":"punk.module_host.side_effect_receipt_writer.operation_evidence.v0.1","receipt_id":"module-receipt-smoke-002","operation_evidence_authority":"advisory"}"#;
    ...
    let receipt_result = write_module_side_effect_receipt_first_active_slice(
        ...
        &storage_root,
        receipt_relative,
        vec!["Operation evidence smoke first writes exact receipt bytes.".to_owned()],
    );
    let evidence_result =
        write_module_side_effect_receipt_writer_operation_evidence_first_active_slice(
            &receipt_result,
            ...
            evidence_relative,
            vec![
                "Operation evidence smoke writes exact evidence bytes to an explicit temporary .punk/runs target only."
                    .to_owned(),
            ],
        );
    let written = fs::read(&evidence_path);
    ...
    let blocked_outside_punk_runs =
        write_module_side_effect_receipt_writer_operation_evidence_first_active_slice(
            &receipt_result,
            operation_evidence_bytes,
            &storage_root,
            std::path::Path::new("operation-evidence/outside.jsonl"),
            vec![],
        );
    let forbidden_runtime_exists = storage_root.join(".punk/events").exists()
        || storage_root.join(".punk/decisions").exists()
        || storage_root.join(".punk/proofs").exists();
    let cleanup_ok = fs::remove_dir_all(&storage_root).is_ok();
    let boundary = module_side_effect_receipt_writer_operation_evidence_write_slice_boundary();

    let vocabulary_ok =
        MODULE_HOST_SIDE_EFFECT_RECEIPT_WRITER_OPERATION_EVIDENCE_WRITE_SLICE_SCHEMA_VERSION
            == "punk.module_host.side_effect_receipt_writer_operation_evidence_write_slice.v0.1"
            && ModuleSideEffectReceiptWriterOperationEvidenceWriteSliceBlocker::ReceiptWriterResultNotSuccessful.as_str()
                == "receipt_writer_result_not_successful"
            && ModuleSideEffectReceiptWriterOperationEvidenceWriteSliceBlocker::TargetPathOutsidePunkRuns.as_str()
                == "target_path_outside_punk_runs"
            && ModuleSideEffectReceiptWriterOperationEvidenceWriteSliceBlocker::ExistingTargetDifferent.as_str()
                == "existing_target_different";
    let write_ok = setup_ok
        && ready_persistence.is_ready()
        && receipt_result.is_success()
        && evidence_result.schema_version
            == MODULE_HOST_SIDE_EFFECT_RECEIPT_WRITER_OPERATION_EVIDENCE_WRITE_SLICE_SCHEMA_VERSION
        && evidence_result.status == ModuleHostStatus::Ready
        && evidence_result.operation_evidence_outcome == ModuleSideEffectReceiptWriterOutcome::Written
        && evidence_result.represents_new_operation_evidence_write()
        && evidence_result.is_success()
        && evidence_result.blockers.is_empty()
        && evidence_result.blockers_fail_closed()
        && matches!(&written, Ok(bytes) if bytes.as_slice() == operation_evidence_bytes)
        && evidence_result.operation_evidence_byte_len == operation_evidence_bytes.len()
        && idempotent.operation_evidence_outcome == ModuleSideEffectReceiptWriterOutcome::Idempotent
        && idempotent.is_idempotent_existing_match()
        && idempotent.is_success()
        && blocked_outside_punk_runs.has_blocker(
            ModuleSideEffectReceiptWriterOperationEvidenceWriteSliceBlocker::TargetPathOutsidePunkRuns,
        )
        && blocked_outside_punk_runs.has_blocker(
            ModuleSideEffectReceiptWriterOperationEvidenceWriteSliceBlocker::OperationEvidenceTargetPathMismatch,
        )
        && blocked_outside_punk_runs.blockers_fail_closed()
        && !storage_root.join("operation-evidence/outside.jsonl").exists();
    let boundary_ok = boundary.implements_operation_evidence_write_slice
        && boundary.requires_successful_receipt_writer_result
        && boundary.requires_explicit_storage_root_path
        && boundary.requires_explicit_punk_runs_operation_evidence_target_relative_path
        && boundary.writes_exact_operation_evidence_bytes
        && boundary.uses_create_new_no_overwrite
        && boundary.reads_filesystem
        && boundary.touches_filesystem
        && boundary.writes_files
        ...
        && !boundary.uses_ide_state_as_authority
        && !boundary.uses_executor_memory_as_authority
        && !boundary.operation_evidence_target_path_is_authority
        && !boundary.storage_root_path_is_authority
        && boundary.operation_evidence_is_non_authoritative
        && boundary.receipt_result_is_non_authoritative
        && boundary.setup_neutral
        && !evidence_result.writes_receipt()
        && evidence_result.writes_punk_runs()
        && evidence_result.reads_filesystem()
    ...
    if vocabulary_ok && write_ok && boundary_ok && no_forbidden_side_effects {
        SmokeEvalCaseResult::pass(
            "eval_module_host_side_effect_receipt_writer_operation_evidence_write_slice_writes_evidence_bytes",
            "module-host side-effect receipt writer operation-evidence write slice writes exact evidence bytes",
            "module-host side-effect receipt writer operation-evidence write slice wrote exact operation-evidence bytes to an explicit temporary .punk/runs target after a successful receipt write while leaving event logs, adapters, policy engines, gate, publishing, comments, pull requests, proofpacks, and acceptance inactive",
        )
    } else {
        SmokeEvalCaseResult::fail(
            "eval_module_host_side_effect_receipt_writer_operation_evidence_write_slice_writes_evidence_bytes",
            "module-host side-effect receipt writer operation-evidence write slice writes exact evidence bytes",
            format!(
                "module-host receipt writer operation-evidence write slice drifted; vocabulary={vocabulary_ok} write={write_ok} boundary={boundary_ok} no_forbidden_side_effects={no_forbidden_side_effects} receipt_outcome={:?} evidence_outcome={:?} blockers={:?} idempotent={:?} blocked={:?} written={:?} setup_ok={setup_ok} cleanup_ok={cleanup_ok}",
                receipt_result.outcome,
                evidence_result.operation_evidence_outcome,
                evidence_result.blockers,
                idempotent.operation_evidence_outcome,
...
mod tests {
    ...
    fn smoke_report_renders_human_output_with_boundaries_and_deferred_notes() {
        ...
        assert!(rendered.contains(
            "assessment: local deterministic smoke harness passed over current contract"
        ));
        assert!(rendered.contains("PubPunk connector profile resolution model"));
        ...
        assert!(rendered.contains("  - id: eval_local_receipt_evidence_handoff_appends_event_refs"));
        assert!(rendered
        ...
        assert!(rendered.contains(
            "  - id: eval_pubpunk_inventory_reader_builds_packet_from_explicit_refs_without_side_effects"
        ));
        assert!(rendered.contains(
            "  - id: eval_pubpunk_inventory_input_packet_is_explicit_and_side_effect_free"
        ));
        assert!(rendered
            .contains("  - id: eval_pubpunk_inventory_assessment_model_is_side_effect_free"));
        assert!(rendered.contains(
            "  - id: eval_pubpunk_channel_connector_profile_resolution_selects_strategy_without_side_effects"
        ));
        assert!(rendered
        ...
        assert!(rendered.contains(
            "  - id: eval_pubpunk_host_handoff_chains_packet_to_receipt_proposal_without_side_effects"
        ));
        assert!(rendered.contains(
            "  - id: eval_pubpunk_publish_request_packet_chains_to_host_side_effect_request"
        ));
        assert!(rendered.contains(
            "  - id: eval_pubpunk_publish_receipt_preflight_chains_to_host_receipt_writer_preflight"
        ));
        assert!(rendered.contains(
            "  - id: eval_pubpunk_publish_receipt_write_handoff_chains_to_host_receipt_writer_write"
        ));
        assert!(rendered.contains(
            "  - id: eval_pubpunk_publish_operation_evidence_handoff_chains_to_host_operation_evidence_write"
        ));
        assert!(rendered.contains(
            "  - id: eval_pubpunk_publish_receipt_evidence_event_handoff_chains_to_local_event_log"
        ));
        assert!(rendered.contains(
        ...
        assert!(rendered.contains(
            "  - id: eval_module_host_side_effect_receipt_writer_file_io_plan_model_is_side_effect_free"
        ));
        assert!(rendered.contains(
            "  - id: eval_module_host_side_effect_receipt_writer_target_storage_policy_model_is_side_effect_free"
        ));
        assert!(rendered.contains(
            "  - id: eval_module_host_side_effect_receipt_writer_host_path_observation_model_is_side_effect_free"
        ));
        assert!(rendered.contains(
        ...
        assert!(rendered.contains(
            "  - id: eval_module_host_side_effect_receipt_writer_first_active_write_slice_writes_receipt_bytes"
        ));
        assert!(rendered.contains(
            "  - id: eval_module_host_side_effect_receipt_writer_operation_evidence_write_slice_writes_evidence_bytes"
        ));
        assert!(rendered.contains(
            "module-host side-effect receipt writer first active write slice smoke case writes exact receipt bytes only to an explicit temporary .punk/runs target without persisting operation evidence"
        ));
        assert!(rendered.contains(
            "module-host side-effect receipt writer operation-evidence write slice smoke case writes exact operation-evidence bytes only to an explicit temporary .punk/runs target after a successful receipt write"
        ));
        assert!(rendered.contains(
        ...
        assert!(rendered.contains(
            "PubPunk publish receipt/evidence event handoff smoke case consumes resolved connector refs before chaining explicit receipt, operation-evidence, writer-result"
        ));
        assert!(
            rendered.contains("PubPunk connector profile resolution smoke case selects API first")
        );
        assert!(rendered.contains(
            "local receipt/evidence event handoff smoke coverage appends only safe receipt and operation-evidence refs to .punk/events/flow.jsonl"
        ));
        assert!(
    ...
    fn smoke_report_renders_json_output_with_v0_1_shape() {
        ...
        assert!(rendered
            .contains("\"case_id\": \"eval_local_receipt_evidence_handoff_appends_event_refs\""));
        assert!(rendered.contains(
            "\"case_id\": \"eval_pubpunk_inventory_reader_builds_packet_from_explicit_refs_without_side_effects\""
        ));
        assert!(rendered.contains(
            "\"case_id\": \"eval_pubpunk_host_handoff_chains_packet_to_receipt_proposal_without_side_effects\""
        ));
        assert!(rendered.contains(
            "\"case_id\": \"eval_pubpunk_publish_request_packet_chains_to_host_side_effect_request\""
        ));
        assert!(rendered.contains(
            "\"case_id\": \"eval_pubpunk_publish_receipt_preflight_chains_to_host_receipt_writer_preflight\""
        ));
        assert!(rendered.contains(
            "\"case_id\": \"eval_pubpunk_publish_receipt_write_handoff_chains_to_host_receipt_writer_write\""
        ));
        assert!(rendered.contains(
            "\"case_id\": \"eval_pubpunk_publish_operation_evidence_handoff_chains_to_host_operation_evidence_write\""
        ));
        assert!(rendered.contains(
            "\"case_id\": \"eval_pubpunk_publish_receipt_evidence_event_handoff_chains_to_local_event_log\""
        ));
        assert!(rendered
