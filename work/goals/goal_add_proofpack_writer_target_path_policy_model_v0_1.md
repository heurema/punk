...
The model classifies explicit target path refs as accepted or rejected and preserves stable reasons for absolute paths, home-relative paths, URL refs, path traversal, ambiguous dot segments, empty segments, unsupported backslashes, and storage-root escape attempts. It maps rejected path policy reasons to file IO diagnostics without reading, canonicalizing, creating, or writing filesystem paths and without turning storage-root refs, target artifact refs, target path refs, indexes, latest pointers, or executor-local state into authority.

No `.punk` runtime state, schema file, CLI command, proofpack writer, filesystem write, gate decision, acceptance claim, provider/model runner, adapter, automation, or `punk init` was added.
