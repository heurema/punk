...
scope:
  ...
  exclude:
    ...
    - ".punk/**"
acceptance:
  - "Advisory Work Ledger Review is completed after referenced artifact verification boundary v0.1 is defined."
  - "Review identifies whether the next safest branch is referenced artifact verification implementation, smoke eval coverage, proofpack writer preparation, another docs/spec guardrail, or another active-core setup step."
...
The boundary distinguishes verification from exact-byte hashing, file IO digest computation, proofpack manifest self-digest, structural link/hash integrity, proofpack writer behavior, gate decisions, and acceptance claims.

Before selecting implementation, smoke eval coverage, writer preparation, runtime storage/schema/CLI work, or gate/eval/proof orchestration, run a short advisory review of the work ledger and open blockers.
