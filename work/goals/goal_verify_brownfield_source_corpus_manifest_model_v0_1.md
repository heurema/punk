...
Done in `work/reports/2026-05-03-brownfield-source-corpus-manifest-model-verification-v0-1.md`.

Verification passed. The model preserves advisory `observed_structure`
authority, repo-relative path validation, no-content defaults, deferred
hash/size policy, canonical source/caution class boundaries, manifest-scoped
evidence refs, forbidden claim fields, and explicit no-scan/no-writer/no-claim
capabilities.

The anti-overclaim grep found no active source inventory, repo scan, manifest
writer, AI summary, claim extraction, contract generation, or `active-core now`
