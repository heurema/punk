...
- `punk-project` provides a Level 0 manual project-memory init scaffold that writes repo-tracked `work/STATUS.md`, an initial ready goal, `work/reports/`, `docs/adr/`, and `knowledge/` starter files with create-new/no-overwrite behavior. It reports conflicts fail-closed and does not create `.punk/` runtime state, flow persistence, contracts, run receipts, gate decisions, proofpacks, or acceptance claims.
...
It does not activate `.punk/` runtime storage, flow/event persistence, contract writing, receipt writing, gate writing, proof writing, or proofpack writing.
